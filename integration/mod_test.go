...
func testMod(t *testing.T, context spec.G, it spec.S) {
	...
	context("when building an app that uses vendored modules in an offline environment", func() {
		var (
			image     occam.Image
			container occam.Container
		...
		it("builds successfully", func() {
			var err error
			source, err = occam.Source(filepath.Join("testdata", "mod_vendor"))
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(logs).To(ContainLines(
				MatchRegexp(fmt.Sprintf(`%s \d+\.\d+\.\d+`, settings.Buildpack.Name)),
				"  Executing build process",
				fmt.Sprintf("    Running 'go build -o /layers/%s/targets/bin -buildmode pie -mod vendor .'", strings.ReplaceAll(settings.Buildpack.ID, "/", "_")),
				MatchRegexp(`      Completed in ([0-9]*(\.[0-9]*)?[a-z]+)+`),
				"",
				"  Assigning launch processes",
				fmt.Sprintf("    web: /layers/%s/targets/bin/mod_vendor", strings.ReplaceAll(settings.Buildpack.ID, "/", "_")),
			))
		})
	})
}
