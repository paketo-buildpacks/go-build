  ...
  - ./second`), 0600)
		...
		it("builds successfully", func() {
			...
			Expect(logs).To(ContainLines(
				...
				fmt.Sprintf("    web (default): /layers/%s/targets/bin/first", strings.ReplaceAll(settings.Buildpack.ID, "/", "_")),
			))
		})
		...
		context("when building an app with target specified via BP_GO_TARGETS env", func() {
			it("builds succesfully and overrides buildpack.yml while still printing a warning", func() {
				...
				Expect(logs).To(ContainLines(
					...
					"",
					"  Assigning launch processes:",
					fmt.Sprintf("    web (default): /layers/%s/targets/bin/third", strings.ReplaceAll(settings.Buildpack.ID, "/", "_")),
				))
			})
		})
	})
